...
class ActivityMainViewModel : BaseViewModel(), KoinComponent {
    ...
    private val superParentCategoryList = MutableLiveData<List<Category>>()
    private val productList = MutableLiveData<List<Product>>()
    private val sortBy = MutableLiveData<SortBy>()
    ...
    fun getParentCategories(context: Context): MutableLiveData<List<Category>> {
        val superParents = repository.getParentCategories(get())
    ...
    fun sortByCategory(context: Context, sortBy: SortBy): MutableLiveData<List<Category>> {
